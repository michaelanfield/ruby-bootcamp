...
describe Wait do
	...
	describe '#until' do
		...
		it 'will expire after 5 seconds by default' do
			expect{ Wait.until { false } }.to raise_error(Wait::TimeOutError)
		end

		it 'will expire after 3 seconds' do
			expect{ Wait.until(:expire_after => 3) { false } }.to raise_error(Wait::TimeOutError)
		end
	end
end